// Implement the structure "Car" (length, ground clearance (landing height),
//engine size, engine power, wheel diameter, body color, gearbox type).
//Create functions for setting values, display values, find values.
...
void addCarToSystem(Vehicle newSerial) {
...
void searchCar(string dataSearch) {
	for (int i = 0; i < countVehicles; i++)
	{
		if (ListVehicles[i].name.find(dataSearch) != string::npos ||
